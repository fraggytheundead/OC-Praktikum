...
//		m_pOs.debug("CreateRobot init: fuege timeout in %i Sekunden hinzu", initTime.sec());
		m_pOs.add_timeout_in(initTime, this, (int*)CREATEROBOT_TASK_INIT);
	}
	else
	{
		initPart2();
...
void Robot::initPart2()
{
	...
	m_pOs.add_task(this, (int*)CREATEROBOT_TASK_PROCESS_DATA);
}
...
{
//	m_pOs.debug("execute, userdata: %i", (int)userdata);
	switch((int)userdata)
	{
	case CREATEROBOT_TASK_PROCESS_DATA:
		processSensorData();
		requestPacket(STREAM_ALL);
		m_pOs.add_task_in(Time(SENSOR_INTERVAL), this, (int*)CREATEROBOT_TASK_PROCESS_DATA);
		break;
